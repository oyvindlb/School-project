...
for (columnName, columnData) in data.iteritems():
    if columnName == "index":
        ...
        data[columnName].replace(0, '', inplace=True)
"""

"""
for row in data.itertuples():
    print(row[:])
    #cursor.execute("INSERT INTO temps (Station_Name, Temperature) VALUES (%s, %s)", (row[1], row[2]))
    cursor.execute("INSERT INTO observations VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", row[:])
connection.commit()
